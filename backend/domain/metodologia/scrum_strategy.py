...
class ScrumStrategy(MetodologiaStrategy):
    """Estrategia concreta para SCRUM"""
    
    def __init__(self, metodologia_obj):
        self.metodologia = metodologia_obj
    
    def obtener_fases(self) -> List[Dict[str, Any]]:
        """Obtiene las fases de SCRUM desde la BD"""
    ...
    def validar_estructura(self) -> bool:
        """
        Validación específica SCRUM.
        SCRUM debe tener al menos Planning, Execution, Review
        """
        fases = Fase.objects.filter(metodologia=self.metodologia)
        return fases.count() >= 3
